...
# ═══════════════════════════════════════════════════════
    ...
    try:
        ...
        df_excel = df.head(10000).copy()
        for col in df_excel.select_dtypes(include=["datetimetz","datetime64[ns, UTC]"]).columns:
            df_excel[col] = df_excel[col].dt.tz_localize(None)
        for col in df_excel.select_dtypes(include=["datetime64"]).columns:
            try:
    ...
    try:
        ...
        def make_pdf():
            ...
            # ── Deep Teal × Saffron palette ─────────────────────────────
            C_BG   = rl_colors.HexColor("#0A1628")
            C_GOLD = rl_colors.HexColor("#FF9933")   # Saffron
            C_G2   = rl_colors.HexColor("#FFB347")   # Light Saffron
            C_TEAL = rl_colors.HexColor("#00C9A7")   # Teal
            C_GRN  = rl_colors.HexColor("#00C9A7")
            C_RED  = rl_colors.HexColor("#FF4D4D")
            C_TXT  = rl_colors.HexColor("#E8F4FF")
            C_MUT  = rl_colors.HexColor("#8BAACC")
            C_DRK  = rl_colors.HexColor("#0D1F3C")
            C_DRK2 = rl_colors.HexColor("#0F2442")

            # ── Font: try to load DejaVu (supports ₹), fallback Helvetica ─
            use_font = "Helvetica"
            try:
                dejavu_url = "https://raw.githubusercontent.com/dejavu-fonts/dejavu-fonts/master/ttf/DejaVuSans.ttf"
                with tempfile.NamedTemporaryFile(suffix=".ttf", delete=False) as tf:
                    urllib.request.urlretrieve(dejavu_url, tf.name)
                    pdfmetrics.registerFont(TTFont("DejaVu", tf.name))
                    use_font = "DejaVu"
            except Exception:
                pass

            # Replace ₹ with "Rs." if font doesn't support it
            def fmt_cur(val):
                sym = currency_symbol()
                if use_font == "Helvetica" and sym == "₹":
                    return f"Rs.{val}"
                return f"{sym}{val}"

            bold_font = use_font + "-Bold" if use_font != "Helvetica" else "Helvetica-Bold"

            title_s = ParagraphStyle("t", parent=styles["Normal"],
                fontSize=26, fontName=bold_font, textColor=C_GOLD,
                alignment=TA_CENTER, spaceAfter=4)
            sub_s = ParagraphStyle("s", parent=styles["Normal"],
                fontSize=10, fontName=use_font, textColor=C_MUT,
                alignment=TA_CENTER, spaceAfter=16)
            h2_s = ParagraphStyle("h2", parent=styles["Normal"],
                fontSize=13, fontName=bold_font, textColor=C_GOLD, spaceAfter=8)
            footer_s = ParagraphStyle("footer", parent=styles["Normal"],
                fontSize=8, fontName=use_font, textColor=C_MUT, alignment=TA_CENTER)

            story.append(Paragraph("RetailIQ AI", title_s))
            story.append(Paragraph("Executive Business Intelligence Report", sub_s))
            story.append(Paragraph(
                f"Generated: {datetime.today().strftime('%d %B %Y')}  "
                f"·  Dataset: {st.session_state.dataset_name or 'Demo'}  "
                f"·  User: {st.session_state.username}",
                sub_s))
            story.append(HRFlowable(width="100%", thickness=1.5,
                                     color=C_GOLD, spaceAfter=16))
            ...
            def tbl_style():
                return TableStyle([
                    ...
                    ("ROWBACKGROUNDS",(0,1), (-1,-1), [C_DRK, C_DRK2]),
                    ("TEXTCOLOR",     (0,1), (0,-1),  C_G2),
                    ("TEXTCOLOR",     (1,1), (-1,-1), C_TXT),
                    ("FONTNAME",      (0,1), (-1,-1), use_font),
                    ("FONTSIZE",      (0,1), (-1,-1), 9),
                    ("GRID",          (0,0), (-1,-1), 0.3, rl_colors.HexColor("#1A3A5C")),
            ...
            kpi_data = [
                ["KPI", "VALUE"],
                ["Total Revenue",    fmt_cur(f"{total_sales:,.2f}")],
                ["Total Profit",     fmt_cur(f"{total_profit:,.2f}")],
                ["Profit Margin",    f"{profit_margin:.1f}%"],
                ["Total Orders",     f"{total_orders:,}"],
                ["Unique Customers", f"{total_cust:,}"],
                ["Avg Order Value",  fmt_cur(f"{total_sales/total_orders:,.2f}") if total_orders else "0"],
            ]
            ...
            if "Category" in df.columns:
                ...
                story.append(Paragraph("Category Performance", h2_s))
                cat_data = [["CATEGORY", "SALES", "PROFIT", "MARGIN"]]
                for cat in df["Category"].unique():
                    cdf = df[df["Category"]==cat]
                    s, p = cdf["Sales"].sum(), cdf["Profit"].sum()
                    cat_data.append([
                        str(cat), fmt_cur(f"{s:,.0f}"),
                        fmt_cur(f"{p:,.0f}"),
                        f"{p/s*100:.1f}%" if s else "0%"])
            ...
            if "Product Name" in df.columns:
                story.append(HRFlowable(width="100%", thickness=0.5,
                                         color=C_TEAL, spaceAfter=8))
                story.append(Paragraph("Top 10 Products by Sales", h2_s))
                top10 = df.groupby("Product Name")["Sales"].sum()\
                          .sort_values(ascending=False).head(10)
                prod_data = [["PRODUCT", "SALES"]]
                for pname, psales in top10.items():
                    prod_data.append([str(pname)[:45], fmt_cur(f"{psales:,.0f}")])
            ...
            # ── Top regions ───────────────────────────────────────────────
            region_col = next((c for c in ["Region","Market","Country"]
                               if c in df.columns), None)
            if region_col:
                story.append(HRFlowable(width="100%", thickness=0.5,
                                         color=C_TEAL, spaceAfter=8))
                story.append(Paragraph(f"Top Regions by Revenue", h2_s))
                reg_data = [["REGION", "SALES", "PROFIT"]]
                for reg, grp in df.groupby(region_col):
                    s, p = grp["Sales"].sum(), grp["Profit"].sum()
                    reg_data.append([str(reg)[:30], fmt_cur(f"{s:,.0f}"), fmt_cur(f"{p:,.0f}")])
                reg_data = sorted(reg_data[1:], key=lambda x: float(x[1].replace("Rs.","").replace(",","").replace(currency_symbol(),"") or 0), reverse=True)[:8]
                rt = Table([["REGION","SALES","PROFIT"]] + reg_data, colWidths=[7*cm, 5*cm, 5*cm])
            ...
            story.append(Paragraph(
                f"RetailIQ AI  ·  Deep Teal x Saffron  ·  {datetime.today().strftime('%d %B %Y')}",
                footer_s))
            doc.build(story)
            buf2.seek(0)
            return buf2.read()
...
# ═══════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'<div style="font-size:0.76rem;color:#B5B5BD;margin-top:4px;">Theme: Deep Teal × Saffron</div>'
        f'</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        'Theme: Deep Teal × Saffron · Enterprise Retail Intelligence Platform<br>'
