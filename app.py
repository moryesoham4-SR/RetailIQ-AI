...
def login_dialog():
    st.markdown(f'<div style="color:{GOLD};font-family:DM Mono,monospace;font-size:0.78rem;margin-bottom:1rem;">Enter your credentials</div>', unsafe_allow_html=True)
    username = st.text_input("Username", placeholder="admin")
    password = st.text_input("Password", type="password", placeholder="password")
    if st.button("Login", use_container_width=True):
        # Simple auth — in production use Supabase
        valid_users = {"admin": hash_pw("admin123"), "demo": hash_pw("demo123")}
        if username in valid_users and valid_users[username] == hash_pw(password):
...
with st.sidebar:
    ...
    if st.button("Logout", use_container_width=True):
        for k in ["logged_in","username","user_id","dataset","dataset_name","col_map","current_page"]:
            st.session_state[k] = False if k == "logged_in" else (None if k in ["user_id","dataset"] else "" if k in ["username","dataset_name"] else {} if k == "col_map" else "🏠 Dashboard")
        st.rerun()
