    ...
    with tab_export:
        ...
        if not entries:
            ...
            if ec2.button("📊 Load into Analytics Now", use_container_width=True):
                ...
                save_dataset(final, st.session_state.dataset_name, {})
        ...
        if "Product Name" in df.columns and "Order Date" in df.columns:
            ...
            if not dead_data:
                ...
                fig_dead = go.Figure(go.Bar(
                    ...
                    hovertemplate="<b>%{y}</b><br>Value: {0}%{{x:,.0f}}<extra></extra>".format(sym)))
                ...
                if "Category" in stock_df.columns if not stock_df.empty else False:
                    ...
                    if not cat_value.empty:
                        ...
                        fig_cat_val = go.Figure(go.Pie(
                            ...
                            textinfo="label+percent", textfont=dict(size=11, color=FONT_C),
                            hovertemplate="<b>%{label}</b><br>{0}%{{value:,.0f}}<extra></extra>".format(sym)))
