...
def save_entries(entries):
    ...
    try:
        import requests as _r
        # Upsert all entries keyed by order_id + product_name
        rows = []
        for e in entries:
            row = dict(e)
...
def load_entries():
    ...
    try:
        import requests as _r
        resp = _r.get(_sb_url("manual_entries", "?user_id=eq.{_sb_user_id()}&order=created_at.asc"),
                      headers=_sb_headers())
        if resp.status_code == 200:
            ...
            for row in rows:
                ...
                for k, v in row.items():
                    ...
                    e[label] = v
                if "Order Date" in e and isinstance(e["Order Date"], str):
                    try: e["Order Date"] = pd.Timestamp(e["Order Date"])
                    except Exception: pass
                # Also keep order_date as Order Date if mapped differently
...
def load_prefs():
    if _sb_available():
        try:
            import requests as _r
            resp = _r.get(_sb_url("retailiq_prefs", "?user_id=eq.{_sb_user_id()}"),
                          headers=_sb_headers())
...
[data-testid="stSidebar"] [role="radiogroup"] label:hover {
    background: var(--gold-dim) !important; border-left-color: var(--gold) !important;
}
[data-testid="stSidebar"] [role="radiogroup"] label:hover p,
[data-testid="stSidebar"] [role="radiogroup"] label:hover span { color: var(--gold2) !important; }
...
.stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 24px rgba(212,175,55,0.35) !important; }
.stButton > button * { color: #0B0B0D !important; -webkit-text-fill-color: #0B0B0D !important; }
.stDownloadButton > button {
    background: var(--bg3) !important; border: 1px solid var(--border) !important;
    color: var(--gold) !important; -webkit-text-fill-color: var(--gold) !important; border-radius: 10px !important;
}
...
[data-testid="metric-container"] {
    ...
    box-shadow: 0 4px 24px rgba(0,0,0,0.35), inset 0 1px 0 rgba(212,175,55,0.08) !important;
}
[data-testid="stMetricValue"] { color: var(--gold2) !important; -webkit-text-fill-color: var(--gold2) !important; font-family: 'DM Mono', monospace !important; }
...
[data-testid="stProgressBar"] > div > div {
    ...
    border-radius: 999px !important; box-shadow: 0 0 8px var(--gold-glow) !important;
}
...
[data-testid="stForm"] {
    ...
    box-shadow: 0 8px 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(212,175,55,0.08) !important;
}

/* Alerts */
[data-testid="stAlert"] {
    background: rgba(212,175,55,0.06) !important; border: 1px solid var(--border) !important;
    border-left: 3px solid var(--gold) !important; border-radius: var(--radius) !important;
}
...
/* Ticker */
.riq-ticker-wrap {
    overflow: hidden; background: rgba(212,175,55,0.05);
...
.riq-card {
    ...
    box-shadow: 0 8px 32px rgba(0,0,0,0.35), inset 0 1px 0 rgba(212,175,55,0.07);
...
.riq-tag {
    ...
    border-radius: 5px; background: var(--gold-dim); color: var(--gold);
...
.riq-page-sub {
    ...
    margin-bottom: 1.4rem; -webkit-text-fill-color: var(--gold);
}
...
.riq-logo {
    ...
    background: linear-gradient(90deg, #D4AF37, #E3C15B);
...
.riq-user-badge {
    ...
    color: var(--gold2); margin-bottom: 0.8rem;
...
AMBER   = "#F59E0B"
BLUE    = "#3B82F6"
PALETTE = [GOLD, GREEN, RED, BLUE, AMBER, GOLD2, "#8B5CF6", "#EC4899"]
...
def kpi_card(tag_txt, val, label, color, spark=""):
    return (
        ...
        f'border:1px solid rgba(212,175,55,0.18);border-top:2px solid {color};'
...
# ═══════════════════════════════════════════════════════
    ...
    def process_dataset(raw, mapping, computed, drop_negatives=True, drop_dupes=True,
        ...
        for dcol in ["Order Date","Ship Date"]:
            if dcol in mapped.columns:
                mapped[dcol] = pd.to_datetime(mapped[dcol], dayfirst=True,
    ...
    with tab1:
        ...
        st.markdown("---")
        if st.button("🔄 Reset to Demo Dataset (Global Superstore)"):
    ...
    with tab2:
        ...
        ds_name = st.session_state.dataset_name or "Global Superstore 2 (Demo)"
        date_from = df["Order Date"].min().strftime("%d %b %Y") if "Order Date" in df.columns else "N/A"
        date_to   = df["Order Date"].max().strftime("%d %b %Y") if "Order Date" in df.columns else "N/A"
        st.markdown(
...
# ═══════════════════════════════════════════════════════
        ...
        with st.form("entry_form", clear_on_submit=True):
            ...
            if unit_price > 0 and quantity > 0:
                ...
                prev_html = (
                    ...
                    f'gap:8px;margin:0.5rem 0 1rem;padding:12px;background:rgba(212,175,55,0.05);' 
    ...
    # ════════════════════════════════════════════════════
    with tab_export:
        st.markdown(f'{tag("EXPORT")}<div class="riq-section-title">Export & Load into Analytics</div>',
                    unsafe_allow_html=True)
        ...
        if not entries:
            ...
            st.markdown(
                f'<div class="riq-card" style="margin-bottom:1rem;">' 
                f'<div style="font-size:0.9rem;color:{GOLD};font-weight:600;margin-bottom:6px;">' 
                f'{len(exp_df):,} entries · {sym}{exp_df["Sales"].sum():,.2f} total sales</div>' 
            ...
            # Load directly into app as active dataset
            if ec2.button("📊 Load into Analytics Now", use_container_width=True):
                final = exp_df.copy()
                if "Profit" not in final.columns:
                    final["Profit"] = 0.0
                if "Order ID" not in final.columns:
                    final["Order ID"] = [f"ORD-{i}" for i in range(len(final))]
                ...
                for nc in ["Sales","Profit","Quantity","Discount"]:
                    if nc in final.columns:
                        final[nc] = pd.to_numeric(final[nc], errors="coerce").fillna(0)
                st.session_state.dataset      = final
                st.session_state.dataset_name = f"Manual Entry — {pd.Timestamp.today().strftime('%d %b %Y')}"
                st.success("✅ Loaded into analytics! Go to any analytics page to explore your data.")
...
# ═══════════════════════════════════════════════════════
    ...
    try:
        ...
        for _, row in seg_summary.iterrows():
            ...
            seg_cards_html += (
                f'<div style="background:rgba(25,25,28,0.8);border:1px solid rgba(212,175,55,0.18);'
...
DATASET: {st.session_state.dataset_name or "Global Superstore"}
        ...
        for msg in st.session_state.chat_history:
            if msg["role"] == "user":
                ...
                st.markdown(
                    ...
                    f'<div style="background:rgba(25,25,28,0.9);border:1px solid rgba(212,175,55,0.18);' 
